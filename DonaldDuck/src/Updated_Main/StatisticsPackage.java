...
public class StatisticsPackage extends DataFrame {
    ...
    public boolean isColumnNumeric(String columnName){
        ...
        for (int column = 0; column < dataFrame[column].length; column++) {
            if (dataFrame[0][column].equals(columnName)) {
                for (int row = 1; row < dataFrame[column].length; row++) {
                    for (int ch = 0; ch < dataFrame[column][row].length(); ch++) {
                        if (Character.isLetter(dataFrame[column][row].charAt(ch)) && !dataFrame[column][row].equals(" ")) {
                            return false;
                        }
                    }
                }
    ...
    public void fillMissingValues(String columnName, double specificValue){
        for (int column = 0; column < dataFrame[column].length; column++) {
            if (dataFrame[0][column].equals(columnName)) {
                for (int row = 1; row < dataFrame.length; row++) {
                    if (dataFrame[row][column].equals(" ")) {
                        dataFrame[row][column] = String.valueOf(specificValue);
    ...
    public void fillMissingValues(String columnName, String specificValue){
        for (int column = 0; column < dataFrame[column].length; column++) {
            if (dataFrame[0][column].equals(columnName)) {
                for (int row = 1; row < dataFrame.length; row++) {
                    if (dataFrame[row][column].equals(" ")) {
                        dataFrame[row][column] = specificValue;
    ...
    public void assignColumnNumericValues(String categoryName){
        ...
        if (specificColumn == null) {
            specificColumn = new double[dataFrame.length - 1];
        }
        for (int i = 0; i < dataFrame[0].length; i++) {
            if (categoryName.equals(dataFrame[0][i])) {
                columnLocation = i;
                break;
            }
        }
        ...
        for (int i = 0; i < dataFrame.length - 1; i++) {
            if (dataFrame[i + 1][columnLocation].equals(" ")) {
                specificColumn[i] = 0;
            } else {
                specificColumn[i] = Double.parseDouble(dataFrame[i + 1][columnLocation]);
            }
        }
    ...
    public void assignColumnNonNumericValues(String categoryName){
        ...
        if (specificColumnString == null) {
            specificColumnString = new String[dataFrame.length - 1];
        }
        for (int i = 0; i < dataFrame[0].length; i++) {
            if (categoryName.equals(dataFrame[0][i])) {
                columnLocation = i;
        ...
        for (int i = 0; i < dataFrame.length - 1; i++) {
            specificColumnString[i] = dataFrame[i + 1][columnLocation];
        }
    }
